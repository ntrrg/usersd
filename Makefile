...
.PHONY: deps
deps: .make/bin/dep
	PATH=".make/bin:$$PATH" dep ensure -v
...
.PHONY: deps-dev
deps-dev: .make/bin/gometalinter
...
.PHONY: qa
qa: .make/bin/gometalinter
	PATH=".make/bin:$$PATH" CGO_ENABLED=0 gometalinter --tests ./ ./internal/... ./pkg/...

.PHONY: test
test:
	go test -race -coverprofile $(coverage_file) -v ./...

.make/bin/gometalinter:
	mkdir -p .make/bin
	...
	cp -a $$(find /tmp/gometalinter-2.0.11-linux-amd64/ -type f -executable) .make/bin/
